...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP) as s:
    ...
    while True:
        ...
        tartByte = bytes_[0]
        if bytes_[0] == 1:
            sys.stdout.write(f"Found Client ip: {adress_[0]} on port: {adress_[1]}"); sys.stdout.flush()
            ...
            if bytes_[0] == 0x1c and adress_ == server_address:
                sys.stdout.write("Got ping response from server"); sys.stdout.flush()
                s.sendto(bytes_, client_address)
        elif bytes_[0] == 5:
            client_address = adress_
            sys.stdout.write(f"Got connection {adress_[0]} on port: {adress_[1]}"); sys.stdout.flush()
            break
